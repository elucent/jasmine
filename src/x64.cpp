...
namespace x64 {
    ...
    void emitargs(const Arg& dest, const Arg& src, Size size, i8 imod = -1) {
        ...
        if (disp) {
            if (disp > -129 && disp < 128) modrm |= 0b01000000; // 8-bit offset
                ...
                fprintf(stderr, "[ERROR] Cannot represent memory offset "
                    "in 32 bits.\n");
                exit(1);
            }
    ...
    void jmp(const Arg& dest, Size size) {
        ...
        if (is_label(dest.type) || is_immediate(dest.type)) {
            if (actual_size == QWORD) {
                fprintf(stderr, "[ERROR] Cannot use 64-bit offset in jump instruction.\n");
                exit(1);
            }
            i64 imm = 0;
    ...
    void call(const Arg& dest, Size size) {
        ...
        if (is_label(dest.type) || is_immediate(dest.type)) {
            if (actual_size == QWORD) {
                fprintf(stderr, "[ERROR] Cannot use 64-bit offset in call instruction.\n");
                exit(1);
            }
            ...
            write_immediate(imm, actual_size);
            target->reference(dest.data.label, relative(actual_size), -4);
        }
        else {
            target->code().write<u8>(0xff);
